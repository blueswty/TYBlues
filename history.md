...
## 2026-07-09 Gallery Detail Layout

### Symptom

相册详情页在改成多图模式后，页面各区块之间留白不足，内容显得过满。
...
## 2026-07-09 Analytics and Comments
...
- Cloudflare Web Analytics 改为使用 Cloudflare Pages 控制台一键启用
- 代码中不再手动注入 Cloudflare 统计脚本
- 评论仍通过 Giscus 配置启用
## 2026-07-09 Comments Visual Refresh

### Goal

璁╄瘎璁哄尯鏇寸粺涓€鍦拌瀺鍏ュ綋鍓嶇背鑹蹭富棰橈紝閬垮厤榛樿璇勮妗嗕笌姝ｆ枃鍗＄墖椋庢牸鍓茶銆?
...
- 璇勮鍖哄灞傛敼涓虹背鑹插崱鐗囧鍣?
- 鏂板 `Discussion` 鏍囩鍜岀畝鐭存槑鏂囨
- Giscus 鍐呴儴涓婚鏀逛负 `noborder_light`锛屽噺灏戦粯璁よ竟妗嗘劅
- 绉诲姩绔篃鍚屾鏀剁揣闂磋窛锛屼繚鎸佺暀鐧芥洿鑷劧
## 2026-07-09 Comments Title Cleanup

### Change

宸插幓鎺夎瘎璁哄尯椤堕儴鐨?`Comments` 鏍囬锛屼粎淇濈暀鏇磋交鐨?`Discussion` 灏忔爣绛惧拰璇存槑鏂囨锛岃璇勮鍖烘洿浣庤皟鍦拌瀺鍏ユ鏂囬槄璇昏妭濂忋€?
## 2026-07-09 Busuanzi Counter

### Change

宸插湪鍏ㄧ珯椤佃剼鎺ュ叆 `busuanzi` 璁块棶缁熻锛屽苟鏄剧ず涓ょ被璁℃暟锛?

- 鏁翠釜绔欑偣鐨勬€昏闂噺
- 褰撳墠椤甸潰鐨勮闂噺

杩欐牱棣栭〉銆佸崥瀹㈡鏂囥€佸懆鍒娿€佺浉鍐屻€佸垪琛ㄩ〉绛夐兘浼氳嚜鍔ㄥ甫涓婅鏁帮紝鏃犻渶鍗曠嫭涓烘瘡绉嶉〉闈㈠啀鍋氫竴娆℃帴鍏ャ€?
## 2026-07-09 Gallery Detail Spacing Fix

### Change

鐩稿唽璇︽儏椤垫敼涓虹嫭绔嬬殑 `album-layout` 瀹藉害瀹瑰櫒锛屼笉鍐嶇洿鎺ョ敤鍋忓鐨?`album-shell` 澶栧眰甯冨眬銆傝繖娆¤皟鏁村悗锛屼袱渚х暀鐧藉凡缁忔仮澶嶏紝浣嗗浘鐗囩綉鏍煎尯鍩熶粛淇濇寔姣旀枃绔犳洿鑸掑睍鐨勫搴︺€?
## 2026-07-09 Typeface Switch

### Change

鍏ㄧ珯姝ｆ枃瀛椾綋浠庣郴缁熸棤琛嚎鍒囨崲涓?`LXGW WenKai / 闇為箙鏂囨エ`锛屽苟閫氳繃 CDN 鍔犺浇 WebFont銆傚悓鏃朵繚鐣欎唬鐮佸潡鐨勭瓑瀹藉瓧浣擄紝閬垮厤褰卞搷浠ｇ爜鍙鎬с€?
